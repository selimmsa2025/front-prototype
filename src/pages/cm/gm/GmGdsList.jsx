  ...
  return (
    <>
      <div className='tit_area'>
        <h3 className='h-tit3'>카탈로그 목록</h3>
      </div>
        ...
        <div className='box_st1 sch_top_wrap'>
          <div className='form_wrap width_half'>
            <div className='form_group'>
              ...
              <select
                name='gdsPvsnMthSeCd'
                value={state.gdsPvsnMthSeCd}
                onChange={handleInputChange}
                className='form_sel sch_type'
              >
                <option value=''>전체</option>
                <option value='A0010001'>기관상품(SaaS형)</option>
                <option value='A0010002'>개별상품(SaaS형)</option>
                <option value='A0010003'>개별상품(설치형)</option>
              </select>
            </div>
            <div className='form_group'>
            ...
            <div className='form_group'>
              <strong className='form_label auto'>API 버전</strong>
              <select
                name='gdsApiVerSn'
            ...
            <div className='form_group'>
              <strong className='form_label auto'>진행상태</strong>
              <select
                name='progressStatus'
              ...
              >
                ...
                <option value='테스트대기'>테스트대기</option>
              </select>
            </div>
      ...
      <div className='board_list'>
        <table className='tstyle_list'>
          <caption>카탈로그 목록</caption>
          <thead>
            <tr>
              <th style={{ cursor: 'pointer' }} onClick={() => changeSort('GDS_PVSN_MTH_SE_NM')}>
                상품 구분{renderSortIcon('GDS_PVSN_MTH_SE_NM')}
              </th>
              <th style={{ cursor: 'pointer' }} onClick={() => changeSort('GDS_TYPE_NM')}>
                상품 유형{renderSortIcon('GDS_TYPE_NM')}
              </th>
              <th style={{ cursor: 'pointer' }} onClick={() => changeSort('GDS_NM')}>
                상품명{renderSortIcon('GDS_NM')}
              </th>
              <th style={{ cursor: 'pointer' }} onClick={() => changeSort('GDS_API_VER_SN')}>
                표준API버전{renderSortIcon('GDS_API_VER_SN')}
              </th>
              <th>등록일자</th>
              <th>진행상태</th>
            </tr>
          ...
          <tbody>
            {dataList.length > 0 ? (
              dataList.map((v, i) => (
                ...
                >
                  <td>{v.gdsPvsnMthSeNm}</td>
                  <td>{v.gdsTypeNm}</td>
                  <td>{v.gdsNm}</td>
                  <td>{v.gdsApiVerSn}</td>
                  <td>{v.frstCrtDt}</td>
                  <td>{v.progressStatus}</td>
                </tr>
              ))
            ) : (
              <tr>
